...
Partial Class _Default
    ...
    Protected Sub btnEnvio_Click(sender As Object, e As EventArgs) Handles btnEnvio.Click
        ...
        Try
            con = New SqlConnection(CStr(Session("sessStrCon")))
            con.Open()
            strSQL = "SELECT MAX(IdOrden) "
            strSQL = strSQL & "FROM tbm_Ordenes"
            sda = New SqlDataAdapter(strSQL, con)
            ...
            'MsgBox(intCodOrd)
            'MsgBox(tx_calen.Text)
            'MsgBox(tx_Cliente.Text)
            'MsgBox(cbo_mesa.SelectedValue)
            'MsgBox(tx_CodighoMesero.Text)

            'MsgBox(CDbl(tx_subtotal.Text).ToString("C3", CultureInfo.CreateSpecificCulture("en-US")))
            'MsgBox(CDbl(tx_total.Text).ToString("C3", CultureInfo.CreateSpecificCulture("en-US")))
            cmd.Parameters.AddWithValue("@IdOrden", intCodOrd)
            cmd.Parameters.AddWithValue("@Fecha", (CDate(tx_calen.Text)).ToString("yyyy-MM-dd"))
            ...
            For i = 0 To dtborden.Rows.Count - 1
                ...
                n = Now.ToString("yyyy-MM-dd HH:mm:ss")
                strSQL = strSQL & ", '" & (CDate(n)).ToString("yyyy-MM-dd HH:mm:ss") & "'"
                strSQL = strSQL & ", '" & (CDate(n).AddSeconds(dtbinsb.Rows(0).Item(0))).ToString("yyyy-MM-dd HH:mm:ss") & "'"
                'strSQL = strSQL & ", ''"
                strSQL = strSQL & ", 'C'"
                strSQL = strSQL & ", " & 0
                strSQL = strSQL & ")"
        ...
        Response.Redirect("MenuMesero.aspx?v1=" & tx_CodighoMesero.Text)
